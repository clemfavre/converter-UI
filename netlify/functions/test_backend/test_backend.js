...
function UpdateContextArea(is_towards_x, dpos, ctx) {
    ...
    let width = is_towards_x ? X_WIDTH : Y_WIDTH; 
    let depth = is_towards_x ? X_DEPTH : Y_DEPTH; 

    let min_x = dpos.x; 
    let max_x = dpos.x + width; 
    let min_y = dpos.y - ((!is_towards_x) ? depth - 1 : 0); 
    let max_y = dpos.y + (is_towards_x ? depth : 0);
...
function ProcessPosition(p, ctx) {
    ...
    else if (ArraysEqual(p.rotation, towards_y) || ArraysEqual(p.rotation, towards_y_alt)) {
        is_towards_x = false;
    }
    else {
