...
export default async (request, context) => {
  try {
    ...
    const ldrText = Buffer.from(fileContent, "base64").toString("utf-8");

    const lines = ldrText.split(/\r?\n/);

    // conversion LDR -> LBCode
    const lbcodeBuffer = ConvertLDR(lines);

    // binary -> base6
    const base64Result = lbcodeBuffer.toString("base64");
...
function ProcessPosition(p, ctx) {
  ...
  if (
    ArraysEqual(p.rotation, towards_x) ||
    ArraysEqual(p.rotation, towards_x_alt)
  ) {
    is_towards_x = true;
  } else if (
    ...
    throw new Error(
      `Error (line ${ctx.line_no}): Unsupported rotation configuration`
    );
  }
  ...
  const encoded_rotation = is_towards_x ? 0 : 1;
...
function ProcessLine(line, ctx) {
  ...
  if (Number.isNaN(type)) {
    throw new Error(`Error (line ${ctx.line_no}): Invalid line type`);
  }

  if (type !== 1) {
    if (type !== 0) {
      console.log(
        `Warning (line ${ctx.line_no}): Unsupported type ${type}, ignoring line`
      );
    }
    return ctx;
  }
  ...
  if (elements.length < 15) {
    throw new Error(
      `Error (line ${ctx.line_no}): Invalid line format (expected >= 15 tokens)`
    );
  }

  const color = parseInt(elements[1], 10);
  if (Number.isNaN(color)) {
    throw new Error(`Error (line ${ctx.line_no}): Invalid color`);
  }
  ...
  for (let i = 0; i < MATRIX_ELEMENTS_COUNT; i++) {
    ...
    if (Number.isNaN(v)) {
      throw new Error(
        `Error (line ${ctx.line_no}): Invalid float at position ${i}`
      );
    }
    matrix.push(v);
  }
